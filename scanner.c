...
int scanner_is_media_file(const char *path) {
    /* Reject obvious in-progress / temp files from downloaders, browsers,
     * torrent clients, rsync, etc. The atomic-rename pattern means we'll
     * see the real filename later. */
    static const char *partial_exts[] = {
        ".nixlypart", ".part", ".partial", ".crdownload", ".tmp",
        ".!ut", ".downloading", ".aria2", ".filepart", NULL
    };
    const char *basename = strrchr(path, '/');
    basename = basename ? basename + 1 : path;
    if (basename[0] == '.') return 0;  /* skip dotfiles, incl. rsync .tmp.XXX */
    for (int i = 0; partial_exts[i]; i++) {
        size_t plen = strlen(partial_exts[i]);
        size_t blen = strlen(basename);
        if (blen >= plen &&
            strcasecmp(basename + blen - plen, partial_exts[i]) == 0) {
            return 0;
        }
    }
...
/* Verify a file is fully written and not still growing.
 * Strategy: stat now, sleep briefly, stat again. If size changed it's
 * still being written. Also enforces a minimum size to skip truncated
 * stubs. FFmpeg probe in scanner_scan_file is the final integrity gate.
 * Returns 1 if complete, 0 if growing/missing/too-small. */
int scanner_file_is_complete(const char *filepath) {
    struct stat st1, st2;
    if (stat(filepath, &st1) != 0) return 0;
    if (!S_ISREG(st1.st_mode)) return 0;
    if (st1.st_size < 1024 * 1024) return 0;  /* < 1 MiB → likely stub */

    /* Brief stability check — catches files still being copied with cp/mv
     * (cross-FS) or written by tools that don't use atomic rename. */
    usleep(800 * 1000);
    if (stat(filepath, &st2) != 0) return 0;
    if (st1.st_size != st2.st_size) return 0;
    if (st1.st_mtime != st2.st_mtime) return 0;

    /* Reject files modified within last 2 seconds — likely still active. */
    if (time(NULL) - st2.st_mtime < 2) return 0;
 ...
 * Looks for paths like: .../TV/<ShowName>[.Year]/Season<N>/<file>
 */
...
int scanner_parse_tv_info(const char *filename, char *show_name, int *season, int *episode, int *year) {
    ...
    *year = 0;
     ...
     * e.g., /media/TV/Breaking Bad/Season 1/Episode 1.mkv */
...
void scanner_rescan_all_tmdb(void) {
    ...
    scrape_begin(&tmdb_progress, "filesystem_scan", server_config.media_path_count);
    for (int i = 0; i < server_config.media_path_count; i++) {
        scanner_scan_directory(server_config.media_paths[i]);
        scrape_update(&tmdb_progress, server_config.media_paths[i], 1);
    }
    scrape_end(&tmdb_progress);
