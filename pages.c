...
"  var shown={co:0,tv:0,mv:0};"
"  document.querySelectorAll('[data-lib]').forEach(function(el){"
"    var hay=el.getAttribute('data-search')||'';"
"    var match=!q||hay.indexOf(q)>=0;"
"    el.style.display=match?'':'none';"
"    var k=el.getAttribute('data-lib');if(k in shown)shown[k]++;"
"  });"
"  var ch=document.getElementById('h-co');if(ch)ch.textContent='Collections · '+shown.co;"
...
"  var [tv,mv,colls]=await Promise.all(["
"    fetch('/api/tvshows').then(r=>r.json()),"
"    fetch('/api/movies').then(r=>r.json()),"
"    fetch('/api/collections').then(r=>r.json()).catch(function(){return [];})]);"
...
"    html+='<h2 id=\"h-co\">Collections · '+colls.length+'</h2><div class=\"grid posters\">';"
"    colls.forEach(function(c){"
"      var art=c.poster||c.backdrop||'';"
"      var hay=(c.name||'').toLowerCase();"
"      html+='<div class=\"card\" data-lib=\"co\" data-search=\"'+esc(hay)+'\" onclick=\"showCollection('+c.id+')\" style=\"cursor:pointer\">'+"
"        '<div class=\"poster-wrap\" style=\"'+posterStyle(art)+'\">'+"
"          (art?'':'<div class=\"poster-empty\">🗂️</div>')+'</div>'+"
"        '<div class=\"body\">'+"
"          '<div class=\"title\">'+esc(c.name)+'</div>'+"
"          '<div class=\"meta\"><span>'+(c.count||0)+' movies</span></div>'+"
"        '</div></div>';"
"    });"
"    html+='</div>';"
"  }"
"  html+='<h2 id=\"h-tv\">TV Shows · '+(tv.length||0)+'</h2><div class=\"grid posters\">';"
...
"  currentView=function(){return showCollection(id);};"
"  view.innerHTML='<p class=\"lead\">Loading…</p>';"
"  var d=await fetch('/api/collections/'+id).then(r=>r.json());"
"  var items=d.items||[];"
"  function posterStyle(p){return p?'background-image:url(/image/'+encodeURIComponent(p)+')':'';}"
"  var html='<a class=\"back\" onclick=\"showTopLevel()\">← Back to library</a>'+"
"    '<h1>'+esc(d.name||'')+'</h1>'+"
"    '<p class=\"lead\">'+items.length+' movie'+(items.length===1?'':'s')+'</p>'+"
...
"      '</div></div>';"
"  });"
"  if(items.length===0)html+='<div class=\"empty\">No movies in this collection</div>';"
"  html+='</div>';"
"  view.innerHTML=html;"
"}"
...
"      '<button class=\"del\" onclick=\"delColl(event,'+x.id+')\" title=\"Delete\">×</button>'+"
"      '</div>';"
"  }).join('');"
"}"
...
"}"
"async function delColl(ev,id){"
"  ev.stopPropagation();"
"  var name=(colls.filter(function(c){return c.id===id;})[0]||{}).name||'';"
