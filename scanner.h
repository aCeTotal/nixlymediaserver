...
/* Check if file is a supported media file (and not a partial/temp file) */
int scanner_is_media_file(const char *path);

/* Verify file is fully written: size stable, > 1 MiB, mtime > 2s old.
 * FFmpeg probe in scanner_scan_file is the final integrity gate. */
