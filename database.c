...
/* Monotonic counter bumped on every write to media table — lets clients
 * detect changes without re-fetching the whole library on each poll. */
static atomic_int db_version = 0;
static inline void bump_version(void) { atomic_fetch_add(&db_version, 1); }

int database_get_version(void) {
    return atomic_load(&db_version);
}
...
int database_init(const char *db_path) {
    ...
    /* Auxiliary tables added by schema migrations. */
    sqlite3_exec(db,
    ...
    /* Versioned migrations: PRAGMA user_version is 0 on fresh DBs, bumped
     * by each migration. To add a v2 migration: check current < 2 → run
     * ALTERs/CREATEs → bump. The IF NOT EXISTS / try-ALTER pattern above
     * makes v1 idempotent so existing DBs converge to the same schema. */
    sqlite3_stmt *st;
    int user_version = 0;
    if (sqlite3_prepare_v2(db, "PRAGMA user_version", -1, &st, NULL) == SQLITE_OK) {
        if (sqlite3_step(st) == SQLITE_ROW) user_version = sqlite3_column_int(st, 0);
        sqlite3_finalize(st);
    }
    if (user_version < 1) {
...
int database_load_blocked_ips(uint32_t **ips, int *count) {
    const char *sql = "SELECT ip FROM blocked_ips";
    sqlite3_stmt *stmt;
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (!txt) continue;
        struct in_addr a;
        if (inet_pton(AF_INET, txt, &a) != 1) continue;
...
int database_add_blocked_ip(const char *ip_str) {
    const char *sql = "INSERT OR IGNORE INTO blocked_ips (ip) VALUES (?)";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) return -1;
...
int database_remove_blocked_ip(const char *ip_str) {
    const char *sql = "DELETE FROM blocked_ips WHERE ip = ?";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) return -1;
    sqlite3_bind_text(stmt, 1, ip_str, -1, SQLITE_STATIC);
    int rc = sqlite3_step(stmt);
...
int database_cleanup_missing(void) {
    ...
    if (missing_count > 0) bump_version();
...
int database_get_stream_meta(int id,
    ...
    const char *sql =
        "SELECT type, COALESCE(tmdb_title, title, ''), "
        "COALESCE(show_name, ''), season, episode, "
        "COALESCE(poster_path, backdrop_path, still_path, ''), size "
        "FROM media WHERE id = ?";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) return -1;
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (season)  *season  = sqlite3_column_int(stmt, 3);
        if (episode) *episode = sqlite3_column_int(stmt, 4);
        if (size)    *size    = sqlite3_column_int64(stmt, 6);
        const char *t = (const char *)sqlite3_column_text(stmt, 1);
        const char *s = (const char *)sqlite3_column_text(stmt, 2);
        const char *p = (const char *)sqlite3_column_text(stmt, 5);
        if (title && title_sz)   { strncpy(title, t ? t : "", title_sz - 1);   title[title_sz - 1] = '\0'; }
        if (show_name && show_sz){ strncpy(show_name, s ? s : "", show_sz - 1); show_name[show_sz - 1] = '\0'; }
        if (poster && poster_sz) { strncpy(poster, p ? p : "", poster_sz - 1); poster[poster_sz - 1] = '\0'; }
        rc = 0;
    }
    sqlite3_finalize(stmt);
    return rc;
