...
static int add_watch_internal(const char *path) {
    if (watch_count >= MAX_WATCHES) {
        fprintf(stderr, "Max watches reached\n");
        return -1;
    }

    /* IN_CREATE kept for directory auto-watch; file-level CREATE is filtered
     * out below — files are only acted on at IN_CLOSE_WRITE (writer done)
     * or IN_MOVED_TO (atomic rename, e.g. from .nixlypart). */
...
int watcher_add_path(const char *path) {
    struct stat st;
    if (stat(path, &st) != 0) {
        ...
        while (i < length) {
            ...
            if (event->len > 0 && callback) {
                ...
                if (watch) {
                    ...
                    /* For files, only fire on writer-close or atomic move-in.
                     * IN_CREATE alone means a 0-byte file just appeared — the
                     * write isn't done yet, so we wait for CLOSE_WRITE. */
                    int is_ready = (event->mask & (IN_MOVED_TO | IN_CLOSE_WRITE)) != 0;
